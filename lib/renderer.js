export function renderTranslation(text) {
  document.getElementById('translation').textContent = text ?? '';
  document.getElementById('tokens-grid').replaceChildren();
...
export function renderTokens(tokens) {
  ...
  for (const token of tokens) {
    grid.appendChild(buildTokenCard(token));
    for (const p of (token.particles ?? [])) {
      grid.appendChild(buildMorphemeCard(p.form, `particle-badge particle-${p.type}`, p.type, p.meaning));
    }
    for (const e of (token.endings ?? [])) {
      grid.appendChild(buildMorphemeCard(`-${e.form}`, `ending-badge ending-${e.type}`, e.type, e.meaning));
    }
  }
}
